...
- Added an `encode_clip()` helper to the WGSL prelude and updated all WebGPU vertex shaders to feed clip-space directly (with only the required depth-range remap) so we preserve the homogeneous `w` while converting OpenGL’s [-1,1] depth to WebGPU’s [0,1]; this keeps every layer (fill, background, line, raster, heatmap, extrusions, symbols, debug, etc.) in sync with the Metal/Vulkan math and avoids the tile drift/missing draw regressions.
...
- Reworked frame pacing so `swap()` no longer blocks on `waitForFrame(500ms)`. The backend now mirrors Vulkan’s fence behaviour: `getCurrentTextureView()` blocks until the prior frame completes (using a condition-variable based `waitForFrame(std::chrono::milliseconds::max())`) before acquiring the next drawable, then presents immediately. Rebuilt (`cmake --build ./build --target mbgl-glfw -- -j8`) and re-ran `timeout 8 ./build/platform/glfw/mbgl-glfw --backend webgpu --style https://demotiles.maplibre.org/style.json --benchmark`; Dawn now tracks Vulkan/Metal motion without the half-second stall or the clipping glitches caused by the earlier polling loop.
